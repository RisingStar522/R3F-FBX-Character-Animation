...
const Character: React.FC<CharacterProps> = ({ camera }) => {
  ...
  const c = useLoader(FBXLoader, "./assets/character/character.fbx");

  c.scale.setScalar(0.1);
  c.traverse((f: { castShadow: boolean; receiveShadow: boolean; }) => {
    f.castShadow = true;
  ...
  const walk = useFBX("./assets/character/walking.fbx");
  ...
  const dance = useFBX("./assets/character/dance.fbx");
  ...
  const calculateIdealLookat = () => {
    const idealLookat = new THREE.Vector3(0, 10, 1000);
  ...
  const characterState = (delta: number) => {
    ...
    console.log(' frameDecceleration = ', frameDecceleration)
